...
function e_singlet(
    ...
    return (
        e_minmin_updown(T, particle_symmetry, spin_symmetry; slave_fermion) -
            e_minmin_downup(T, particle_symmetry, spin_symmetry; slave_fermion)
    ) / sqrt(2)
end
...
function e_number_up(
        T::Type{<:Number}, ::Type{Trivial}, ::Type{Trivial};
        slave_fermion::Bool = false
    )
