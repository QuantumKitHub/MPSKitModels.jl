...
quantum_chemistry_hamiltonian(E0,K,V) = _quantum_chemistry_hamiltonian(E0,K,V)[1]

function _quantum_chemistry_hamiltonian(E0,K,V,Elt=ComplexF64)
    basis_size = size(K,1);
    ...
    for i in 1:basis_size
        ...
        for loc in i+1:basis_size
            hamdat[loc,map_1[1,i],map_1[1,i]] = Elt(1)
            hamdat[loc,map_1[2,i],map_1[2,i]] = Elt(1)
        end

        for loc in 1:i-1
            hamdat[loc,map_4[1,i],map_4[1,i]] = Elt(1)
            hamdat[loc,map_4[2,i],map_4[2,i]] = Elt(1)
        end
    end
    ...
    for i in 1:basis_size
        ...
        for loc in i+1:basis_size
            hamdat[loc,map_2[1,i,1,i],map_2[1,i,1,i]] = Elt(1);
            hamdat[loc,map_2[2,i,1,i],map_2[2,i,1,i]] = Elt(1);
            hamdat[loc,map_2[2,i,2,i],map_2[2,i,2,i]] = Elt(1);

            hamdat[loc,map_3[2,i,1,i],map_3[2,i,1,i]] = Elt(1);
        end
    end
    ...
    for i in 1:basis_size, j in i+1:basis_size
        ...
        for k in j+1:basis_size
            hamdat[k,map_2[1,i,1,j],map_2[1,i,1,j]] = Elt(1);
            hamdat[k,map_2[1,i,2,j],map_2[1,i,2,j]] = Elt(1);
            hamdat[k,map_2[2,i,2,j],map_2[2,i,2,j]] = Elt(1);
            hamdat[k,map_2[2,i,1,j],map_2[2,i,1,j]] = Elt(1);

            hamdat[k,map_3[1,i,2,j],map_3[1,i,2,j]] = Elt(1);
            hamdat[k,map_3[2,i,1,j],map_3[2,i,1,j]] = Elt(1);
        end
    end
    ...
    for i in 1:basis_size,j in i+1:basis_size,k in j+1:basis_size
        ...
        # p p L R
        #@plansor ppLR[-1 -2;-3 -4] := ap[1 -2;-4]*bp[-1;-3 1]
        hamdat[j,map_1[2,i],map_4[2,k]] += V[j,j,i,k]*ppLR
    ...
    @time th = MPOHamiltonian(convert(Array{Union{Elt,typeof(ut_ap)},3},map(x->x == InitialValue(+) ? Elt(0) : x,hamdat)));
    #th = MPOHamiltonian(MPSKit.remove_orphans(th.data));
    th+fill(Elt(E0)/basis_size,basis_size),(map_1,map_2,map_3,map_4)
end