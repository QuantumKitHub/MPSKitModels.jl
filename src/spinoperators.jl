...
function sigma_x(elt::Type{<:Number}, ::Type{ℤ₂}; spin=1 // 2)
    spin == 1 // 2 || error("not defined for spin != 1 // 2")
    pspace = Z2Space(0 => 1, 1 => 1)
    vspace = Z2Space(1 => 2)
    s = TensorMap(zeros, elt, pspace ⊗ vspace ← pspace)
    blocks(s)[Z2Irrep(0)][1] = one(elt)
    blocks(s)[Z2Irrep(1)][1] = one(elt)
    return s
end

function sigma_y(elt::Type{<:Number}, ::Type{U₁}; spin=1 // 2)
end
...
function sigma_y(elt::Type{<:ComplexF64}, ::Type{ℤ₂}; spin=1 // 2)
    spin == 1 // 2 || error("not defined for spin != 1 // 2")
    pspace = Z2Space(0 => 1, 1 => 1)
    vspace = Z2Space(1 => 2)
    s = TensorMap(zeros, elt, pspace ⊗ vspace ← pspace)
    blocks(s)[Z2Irrep(0)][2] = -one(elt)im
    blocks(s)[Z2Irrep(1)][2] = one(elt)im
    return s
end
...
function sigma_plus(elt::Type{<:Number}, ::Type{ℤ₂}; spin=1 // 2)
    pspace = Z2Space(0 => 1, 1 => 1)
    vspace = Z2Space(1 => 2)
    s = TensorMap(zeros, elt, pspace ⊗ vspace ← pspace)
    blocks(s)[Z2Irrep(0)][1] = one(elt)
...
function sigma_min(elt::Type{<:Number}, ::Type{ℤ₂}; spin=1 // 2)
    pspace = Z2Space(0 => 1, 1 => 1)
    vspace = Z2Space(1 => 2)
    s = TensorMap(zeros, elt, pspace ⊗ vspace ← pspace)
    blocks(s)[Z2Irrep(1)][2] = one(elt)
    return s
end
...
function sigma_xx(elt::Type{<:Number}=ComplexF64, ::Type{ℤ{1}}=ℤ{1}; spin=1 // 2)
    return sigma_x(elt, ℤ{1}; spin=spin) ⊗ sigma_x(elt, ℤ{1}; spin=spin)
end

function sigma_xx(elt::Type{<:Number}, symmetry::Type{G}; spin=1 // 2) where {G}
    X = sigma_x(elt, symmetry; spin=spin)
    return @tensor Sxx[-1 -2; -3 -4] := X[-1 1; -3] * conj(X[-4 1; -2])
end
...
See also [`σʸʸ`](@ref)
"""
function sigma_yy(elt::Type{<:Number}=ComplexF64, ::Type{ℤ{1}}=ℤ{1}; spin=1 // 2)
    return sigma_y(elt, ℤ{1}; spin=spin) ⊗ sigma_y(elt, ℤ{1}; spin=spin)
end

function sigma_yy(elt::Type{<:Number}, symmetry::Type{G}; spin=1 // 2) where {G}
    Y = sigma_y(elt, symmetry; spin=spin)
    return @tensor Syy[-1 -2; -3 -4] := Y[-1 1; -3] * conj(Y[-4 1; -2])
end
